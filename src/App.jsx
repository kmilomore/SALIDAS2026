...
export default function App() {
  ...
  const derivedMetrics = useMemo(() => {
    ...
    const pme2025CoverageChart = [
      { name: 'Escuelas con cobertura 2025', value: new Set(concreted2025.map((item) => item.rbd)).size },
      { name: 'Escuelas sin cobertura 2025', value: new Set(pending2025.map((item) => item.rbd)).size },
    ];

    const pme2025SchoolsChart = [...schools2025Map.values()]
      .sort((left, right) => right.outings - left.outings || String(left.name || '').localeCompare(String(right.name || ''), 'es'))
      .slice(0, 10)
      .map((item) => ({
        name: item.name,
        commune: item.commune,
